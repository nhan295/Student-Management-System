  ...
  const handleConfirm = async () => {
    ...
    try {
      if (action === "addStudent") {
        await axios.post("http://localhost:3000/api/v1/students", data);
        ...
        await axios.post("http://localhost:3000/api/v1/courses", data);
        toast.success("Thêm khóa học mới thành công!");
        setNewCourse({ course_name: "", start_year: "", end_year: "" });
        fetchCourses();
        setShowCourseForm(false);
      } else if (action === "addClass") {
        await axios.post("http://localhost:3000/api/v1/classes", data);
